...
def main(args):
    ...
    model_memory = torch.cuda.max_memory_allocated(0)
    wandb.log({f'{args.task}/model_memory': model_memory})
    raw_inputs = load_task_dataset(args)
    model_inputs = []

    print(model_name)
    max_memory = np.zeros(args.sample)
    ...
    with torch.no_grad():
        ...
        for i, d in enumerate(model_inputs[:args.sample]):
            ...
            max_memory[i] = curr_max_memory

    print(f"*** Average max memory for {model_name} at {args.task}: {max_memory.mean()} ms")
    wandb.log({f'{args.task}/max_memory_allocated': max_memory.mean()})
