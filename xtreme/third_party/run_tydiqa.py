...
from tydiqa_model import CanineTyDiQAModel
from processors.tydi_char_splitter import CharacterSplitter
...
def evaluate(args, model, tokenizer, prefix="", language='en', lang2id=None):
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    model.eval()
    batch = tuple(t.to(args.device) for t in batch)
    input_ids, input_masks, segment_ids, example_index, start_offset, end_offset = batch
