...
class BertTyDiQAModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size, config.num_labels, bias=True)
        # self.pooled_outputs = nn.Linear(config.hidden_size, config.hidden_size)
        # self.tanh = nn.Tanh()
        self.answer_type_output_weights = nn.Linear(config.hidden_size, len(data.AnswerType), bias=True)
        self.init_weights()
    ...
    def forward(
        ...
        # pooled_output = self.pooled_outputs(outputs[1])         # hidden state of first token ([CLS]) [B x H]
        # pooled_output = self.tanh(pooled_output)                # [B x H]
        answer_type_logits = self.answer_type_output_weights(pooled_output)     # [B x Q]
...
class BertDebugTyDiQAModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
